...
template <class T>
promise<T>::promise()
    : impl_(std::make_shared<detail::future_state_impl<T>>()), future_impl_(impl_) {}

template <class T>
promise<T>::promise(promise&& other) noexcept
    : impl_(std::move(other.impl_)), future_impl_(std::move(other.future_impl_)) {}
...
template <class T>
auto promise<T>::set_error(std::error_code ec) -> void {
    assert(impl_);
    impl_->set_value(ec);
    impl_.reset();
}
...
auto promise<T>::destroy() -> void {
    if (impl_) {
        impl_->set_value(std::error_code(asio::error::operation_aborted));
        impl_.reset();
    }
    if (future_impl_)
        future_impl_.reset();
